...
def print_prediction(prediction, rev_vocab, postprocessor=None, with_context=False, context_char_limit=500):
    ...
    answers_probs = zip(prediction.answer_strings, prediction.answer_probs)
    if postprocessor is not None:
        answers_probs = postprocessor.process(answers_probs)
    print_list(answers_probs)
    if with_context:
